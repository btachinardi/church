...
**98 purist agents. 18 crusades. Zero tolerance.**
...
[![98 Agents](https://img.shields.io/badge/agents-98-orange.svg)](#purist-agents)
[![18 Crusades](https://img.shields.io/badge/crusades-18-red.svg)](#crusade-commands)
...
- **Two-Tier Agent System** — 18 generic purists for direct invocation + 80 specialists for crusade deployment
- **18 Crusade Commands** — One `/church:*-crusade` command per domain, each orchestrating 4-6 specialist agents
...
Each generic purist covers its full domain. During crusades, **80 specialist agents** (4-6 per domain) handle narrower concerns for deeper analysis.
...
`type` · `arch` · `test` · `react` · `git` · `dead` · `naming` · `size` · `secret` · `dep` · `observability` · `a11y` · `copy` · `adaptive` · `python` · `rust` · `kotlin` · `compose`
