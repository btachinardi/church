...
export const reactCrusade: CrusadeDetail = {
  ...
  howItWorks: [
    { phase: 'Reconnaissance', description: 'Scans the battlefield by globbing all .tsx component files, counting hooks, stores, and providers. Quick-greps for red flags -- fetch in useEffect, suppressed exhaustive-deps, excessive useState -- to estimate the severity of heresy before deploying the squads.' },
    { phase: 'Squad Formation', description: 'Five specialist squads are assembled, each carrying only the doctrine it needs. The Arch Purist classifies tiers. The Hooks Purist audits effects. The State Purist enforces the Great Separation. The Data Purist hunts race conditions. The Perf Purist eliminates wasted renders.' },
    ...
    { phase: 'Consolidated Report', description: 'A unified battle report is generated with component tier census, severity breakdown, Unifying Lens compliance audit, top priority targets with specific file paths, and a final victory declaration based on whether CRITICAL heresies have been purged.' },
    { phase: 'Victory Judgment', description: 'If zero CRITICAL issues remain, the Immutable State blesses the codebase. If heresies persist, the faithful are given specific file paths and refactoring steps to continue the purge until every component is PURE, every effect is CLEAN, and every state boundary is SACRED.' },
  ],
} as const;
