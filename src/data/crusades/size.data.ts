...
export const sizeCrusade: CrusadeDetail = {
  ...
  howItWorks: [
    'Reconnaissance: Scans every .ts and .tsx file in scope, counting lines and classifying each by type -- component, hook, service, entity, utility, and beyond. Applies sacred thresholds per file type, exempts barrel files and marked exclusions, and produces a dramatic Horror Report ranking every offender by severity.',
    'Squad Assignment: Each bloated file is assigned to one of four surgical squads based on its suffix -- Component Surgery for frontend files, Service Surgery for backend logic, Domain Surgery for entities and aggregates, Utility Surgery for helpers and infrastructure. Files that match no specialist pattern fall to the Utility Squad as overflow.',
    'Parallel Deployment: All four surgical squads are launched simultaneously via the Task tool in a single message. Each specialist agent carries only the doctrine for its file type, analyzing assigned files for distinct responsibilities, function lengths, nesting depth, and import counts.',
    'Surgical Analysis: Each squad produces detailed split plans with specific line ranges, extraction strategies, new file names, and post-surgery size estimates. Plans are synthesized into a Master Surgical Plan showing total files to split, new files to create, and estimated outcomes.',
    'Execution (--split mode): Upon confirmation, squads execute the splits in parallel -- creating new files, moving code, updating every import across the codebase, and verifying with the TypeScript compiler. No import is left broken. No circular dependency is tolerated.',
    'Victory Report: A final re-count verifies that all previously bloated files now meet their thresholds. TypeScript compilation and tests confirm the codebase is whole. The bloat has been purged. Maintainability is restored.',
  ],
} as const;
