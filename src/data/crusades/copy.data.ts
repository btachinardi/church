...
export const copyCrusade: CrusadeDetail = {
  ...
  howItWorks: [
    ...
    'Squad Assignment: Each violation is assigned to one of four specialist squads — Microcopy Exorcists handle buttons and UI text, Transactional Templars handle emails and SMS, Headline Inquisitors handle hero sections and CTAs, Framework Enforcers handle persuasion structure and benefit mapping.',
    'Parallel Deployment: All four specialist squads are launched simultaneously via the Task tool in a single message. Each agent carries only the doctrine for its domain — microcopy principles, deliverability laws, headline formulas, or persuasion frameworks — ensuring focused, expert analysis.',
    'Copy Audit: Each squad reads assigned files, identifies user goals and emotional states, diagnoses copy sins, and proposes specific rewrites using proven frameworks (AIDA, PAS, FAB, 4Ps). Plans include before/after examples, psychological principles, and estimated conversion impact.',
    'Execution (--write mode): Upon confirmation, squads rewrite copy in parallel using the Edit tool — buttons become specific, error messages include next steps, headlines state benefits, features connect to user gains. No edit occurs without the user approving the plan first.',
    'Victory Report: A final re-scan verifies all violations are fixed. The report shows before/after metrics, estimated conversion lift (15-30%), reduced cart abandonment (10-20%), and improved email deliverability (+15-25%). Vague copy has been clarified. Robotic messages have been humanized.',
  ],
} as const;
