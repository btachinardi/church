...
export const observabilityCrusade: CrusadeDetail = {
  ...
  howItWorks: [
    'The Darkness Survey: The crusade begins by scanning the entire codebase for every sin against observability -- empty catch blocks, bare console calls, untraced endpoints, missing correlation IDs, swallowed errors, unstructured logs, and missing health checks. Each violation is catalogued with file path, line number, and severity.',
    'The Darkness Report: All findings are aggregated into a structured Darkness Report, categorized by sin type and severity (CRITICAL, HIGH, MEDIUM). The first three examples of each sin are displayed with code snippets and consequences explained in full prophetic voice.',
    'Squad Formation: Files are partitioned by sin category and assigned to specialist squads based on codebase size -- two squads for small codebases, three for medium, and four fully independent squads for large operations. Each squad receives only the files and doctrine relevant to its sacred mission.',
    'Parallel Deployment: All specialist squads -- Logging Purification, Error Handling, Distributed Tracing, and Health & Metrics -- are launched simultaneously via the Task tool in a single message. Each squad carries its specialist agent and operates with surgical precision on its assigned files.',
    'Verification and Illumination: After all squads return, the darkness detection scans are re-run to count remaining sins. The Illumination Report reveals how many sins were eliminated, what new light sources were created (middleware, interceptors, health endpoints), and the final illumination percentage.',
    'Remaining Darkness: Any sins requiring human judgment -- complex error handling logic, legacy code dependencies, or external library integration -- are highlighted with specific file paths and actionable guidance for manual resolution.',
  ],
} as const;
