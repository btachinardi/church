...
**Python projects** must have mutmut configured:
- `[tool.mutmut]` section in `pyproject.toml`, or `mutmut` in `setup.cfg`
- `mutmut` in dev dependencies (`pyproject.toml` optional-dependencies or `requirements-dev.txt`)
...
| mutmut not in dev dependencies (Python) | CRITICAL |
